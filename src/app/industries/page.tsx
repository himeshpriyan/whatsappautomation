  ...
  return (
    <div className="pt-8 pb-24 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Breadcrumb items={[{ label: "Industry Solutions" }]} />
        {/* Header */}
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl border text-center transition-all flex flex-col items-center justify-center gap-2 cursor-pointer ${
                  isSelected
                    ? "bg-emerald-50 border-emerald-500 shadow-md -translate-y-1 text-emerald-950"
                    : "bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50 text-slate-700 shadow-xs"
                }`}
              >
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${
                    isSelected ? "bg-[#25D366] text-slate-950 font-bold" : "bg-slate-100 text-slate-600"
                  }`}
                >
                ...
                <span
                  className={`text-xs font-bold ${
                    isSelected ? "text-slate-950 font-extrabold" : "text-slate-600 hover:text-slate-900"
                  }`}
                >
                  {item.name.split("&")[0]}
                </span>
        ...
        <div className="p-8 sm:p-12 rounded-3xl bg-white border border-slate-200 shadow-xl mb-24 relative overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-100 mb-8">
            <div>
              ...
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900">{current.headline}</h2>
            </div>
            <Button
              href="/signup"
              variant="primary"
          ...
          <p className="text-sm sm:text-base text-slate-600 mb-10 max-w-3xl leading-relaxed">
            {current.description}
          </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {current.benefits.map((benefit, idx) => (
              <div
                key={idx}
                className="p-6 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col justify-between space-y-4 hover:border-emerald-500/40 transition-colors shadow-xs"
              >
                <div>
                  ...
                  <div className="bg-[#E7F8E8] rounded-2xl rounded-tr-none p-4 text-xs text-slate-900 shadow-xs space-y-2 border border-emerald-300/60">
                    <div className="font-bold text-emerald-950">{benefit.templateMessage.header}</div>
                    <div className="text-[11px] text-slate-700 leading-relaxed">
                      {benefit.templateMessage.body}
                    </div>
                    <div className="pt-1">
                      <div className="bg-white text-emerald-800 font-bold text-[11px] py-1.5 px-3 rounded-lg text-center border border-emerald-200 shadow-2xs">
                ...
                <div className="pt-3 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-500 font-medium">
                  <span className="flex items-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" /> Standard Cloud API Format
                  </span>
                  <span className="text-emerald-700 font-bold">1-Click Clone</span>
                </div>
              </div>
            ))}
          </div>
